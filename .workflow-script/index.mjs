...
// const agent = new HttpProxyAgent('http://127.0.0.1:10809')
...
async function main() {
  ...
  const getManifestPromises = list.map(plugin =>
    limit(async () => {
      console.log(`获取 ${plugin.repo}`)
      /**@type {Manifest} */
      const manifest = await superagent
        .get(getRawUrl(plugin, 'manifest.json'))
        .then(res => JSON.parse(res.text))
        .catch(async e => {
          if (e.status == 404) {
            return await superagent
              .get(getRawUrl(plugin, 'package.json'))
              .then(res => {
      ...
      allManifest[plugin.repo] = manifest || null
      if (manifest) {
        meta.success++
  ...
  writeFileSync(join(outPath, 'all-manifest.json'), JSON.stringify(allManifest))
  ...
  cpSync(join(__dirname, 'public/index.html'), join(outPath, 'index.html'), { force: true })
